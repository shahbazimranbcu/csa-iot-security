"""
==========================================================================
 CSA ADVERSARIAL ROBUSTNESS — TON-IoT HOST DATASET
...
FIXES:
  ...
  2. hill-climb early exit: same direction as success logic
  3. safe_load_model: handles sklearn version mismatch
"""
...
MODEL_FILE = "host_normal.pkl"
# MODEL_FILE = "host_adver.pkl"

DATA_FILE  = "host_dataset_train.csv"
TYPE_COL   = "type"
OUTPUT_DIR = Path("results_ton_host")
...
ALL_FEATURES = ["PID","TRUN","TSLPI","TSLPU","POLI","NICE","PRI","RTPR","CPUNR","Status","EXC","CPU"]
SELECTED_FEATURES = ["CPU","TRUN","TSLPI","CPUNR","Status","EXC","PRI","NICE"]
FEAT_BOUNDS = {"CPU":(0.0,4.0),"TRUN":(0.0,9.0),"TSLPI":(0.0,67.0),
               "CPUNR":(0.0,5.0),"Status":(0.0,4.0),"EXC":(0.0,100.0),
               "PRI":(0.0,139.0),"NICE":(0.0,20.0)}
CORRELATION_RULES = [("TRUN","CPU",0.01,+1)]
CORRELATION_THRESHOLD = 1
STRIDE_SIGNATURE = {"dos":["CPU","TRUN"],"mitm":["TSLPI","EXC"]}
...
_miss = [f for f in SELECTED_FEATURES if f not in ALL_FEATURES]
if _miss: raise ValueError(f"SELECTED_FEATURES not in ALL_FEATURES: {_miss}")
...
def safe_load_model(model_file, data_file):
    ...
    try:
        ...
        ts = df[TYPE_COL].str.lower().str.strip()
        ...
        log.info("  Training RF n_estimators=%d on %d samples...", n_est, len(X))
...
def load_data():
    ...
    labels = [CLASS_NAMES.index(str(t).lower().strip())
              if str(t).lower().strip() in CLASS_NAMES else -1
              for t in ts[mask].values]
    y_flt = np.array(labels, dtype=np.int32)
    valid = y_flt >= 0
    X_flt, y_flt = X_flt[valid], y_flt[valid]
    ...
    for z in [2.5,3.0,3.5,4.0,4.5,5.0,6.0]:
        ...
        for s in Xn:
            v=sum(1 for j,fn in enumerate(SELECTED_FEATURES)
                  ...
                  /bls[NORMAL_IDX][fn]["ewma_std"]>z)
            if v>=len(SELECTED_FEATURES)//2: flagged+=1
        fpr=flagged/max(len(Xn),1)
        log.info("  z=%.1f FPR=%.2f%%",z,fpr*100)
        if fpr<=0.05 and bf>0.05: bz,bf=z,fpr
    log.info("Selected z=%.1f (FPR=%.2f%%)",bz,bf*100)
    return bz
...
def rule_sstride(xa, ci, bls):
    ...
    for fn in sf:
        ...
        if float(xa[SELECTED_IDX[fm[fn]]])>=bl[fn]["raw_mean"]*STRIDE_MULT:
            return True
    return False
...
def csa_detect(xo, xa, ci, iso, bls, zt, mode="FULL"):
    ...
    viols= sum([not tok,not cok,not sok])
    ...
    det=(not cok) or (viols>=2) or iflag
    rs=[]
...
def make_blackbox(model):
    ...
    return BlackBoxClassifier(pfn,input_shape=(len(ALL_FEATURES),),
                              nb_classes=len(CLASS_NAMES),clip_values=(0.0,1e6))

def adaptive_loss(xa, xo, ci, model, bls, zt):
    proba=model.predict_proba(xa.reshape(1,-1))[0]
    ml=1.0-proba[ci]
    bl=bls.get(ci,{})
...
def mean_ci(vals, conf=0.95):
    ...
    if n<2: return m,0.0
    h=stats.sem(vals)*stats.t.ppf((1+conf)/2,df=n-1)
    return m,float(h)

def run_trial(hsj, model, Xe, ye, iso, bls, zt, sf, rng,
              mode, amode, tnum, ocsv, cols):
    ...
    print(f"  {'─'*100}")
    print(f"  {'#':>5}  {'Class':>10}  {'PredAfter':>10}  {'Result':>9}  "
          f"{'Iters':>6}  {'ML-ASR':>7}  {'CSA-ASR':>8}  {'CSA':>6}  {'Reason':<25}")
    print(f"  {'─'*5}  {'─'*10}  {'─'*10}  {'─'*9}  {'─'*6}  {'─'*7}  {'─'*8}  {'─'*6}  {'─'*25}")
    for cn in EXECUTION_ORDER:
        ci=CLASS_NAMES.index(cn)
        idx=np.where(ye==ci)[0]
        ...
        for xr in Xe[chosen]:
            ...
            print(f"  {total:>5}  {cn:>10}  {CLASS_NAMES[pa]:>10}  "
    ...
    d=max(total,1)
    ml=fooled/d*100; csa=csa_pass/d*100
...
def main():
    ...
    bls=fit_baselines(Xv,yv)
    zt=calibrate_zscore(Xv,yv,bls)
    ...
    AMODES=["FULL","NO_TMVA","NO_CORR","NO_STRIDE","NO_IF"]
    ATKS=["oblivious","adaptive"]
    cols=["trial","mode","attack_mode","scaling_factor","class_true",
          "success","csa_detected","csa_net_success","csa_reason",
          "iterations_used","pred_before","pred_after","live_ml_asr","live_csa_asr"]
    ...
    for atk in ATKS:
        ...
        for sf in sfs:
            for mode in AMODES:
                ...
                mm,hm=mean_ci(mll); mc,hc=mean_ci(csall)
                j=mm*mc/100.0
    ...
    sc=OUTPUT_DIR/f"{mn}_summary.csv"; df.to_csv(sc,index=False); log.info("CSV → %s",sc)
